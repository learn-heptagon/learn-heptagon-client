...
let load_interp (console_div_id: string) (interp_div_id: string) (prog: Obc.program) int =
  !stop_fun ();
  (try Page.clear_div interp_div_id with _ -> ());
  match int with
  | Chronogram ->
    let names = List.filter_map (function Pclass cl -> Some cl.cd_name.name | _ -> None) prog.p_desc in
    Page.create_select interp_div_id names (choose_default names !old_node)
      (fun cname ->
        old_node := Some cname;
        ...
        Page.show_chronogram console_div_id interp_div_id st reset step
      )
  | Simulator simul ->
